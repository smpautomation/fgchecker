<script setup>
/**
 * FGChecker.vue — Landing page
 * Stack: Laravel + Inertia + Vue 3 (<script setup>)
 *
 * Expected backend routes (adjust names to match your routes/web.php):
 *   GET  /records?from=YYYY-MM-DD&to=YYYY-MM-DD   -> JSON { data: [...] }
 *   GET  /export?format=xlsx|csv&from=...&to=...  -> file download
 *   GET  /scan   -> Scan.vue
 *   GET  /print  -> Print.vue
 *
 * The backend is responsible for reading from the current-year table
 * (fgchecker{YYYY}) and creating it as a copy of fgchecker_format on first write.
 */
...
/**
 * Type comes straight from the backend, which joins fgchecker_monitoring_process
 * (Process, Type) against this record's Result. Falls back to NOT GOOD if a
 * process has no matching Type row, so unrecognized values fail loud rather
 * than silently reading as "good".
 */
      ...
      <Transition name="reveal">
        <div v-if="showTable" class="panel">
          ...
          <div class="filter-bar">
            ...
            <button class="btn btn-outline" @click="loadRecords" :disabled="loading">Apply Filter</button>
...
.btn-chip:hover { background: var(--accent); color: var(--accent-ink); border-color: var(--accent); }
...
.stamp-good { color: var(--pass); transform: rotate(-7deg); }
.stamp-notgood { color: var(--fail); transform: rotate(4deg); }
.stamp-reload { color: var(--reload); transform: rotate(-3deg); font-size: 1.05rem; }
...
.section-sub { margin: 0; color: var(--text-dim); font-size: 0.95rem; }

.reveal-enter-active { transition: opacity .3s ease, transform .3s ease; }
...
.export-label { color: var(--text-dim); font-size: 0.85rem; font-weight: 500; }

.counters { display: flex; gap: 14px; margin-bottom: 20px; flex-wrap: wrap; }
.counter {
  background: var(--surface-2);
  border: 1px solid var(--border);
  border-radius: 10px;
...
.counter-num { font-family: 'Big Shoulders Display', sans-serif; font-size: 1.8rem; font-weight: 700; line-height: 1; }
.counter-label { color: var(--text-dim); font-size: 0.82rem; margin-top: 4px; }
...
.error-note { color: var(--fail); font-weight: 500; margin: 0 0 14px; }
...
/* Wide tablets (landscape Galaxy Tab / iPad) — hero/table already scale via clamp() */
@media (max-width: 1024px) {
  .stamp-row { right: 24px; top: 32px; }
  .stamp { font-size: 1.1rem; padding: 8px 14px; }
}

/* Portrait tablets and down: stack the filter bar, widen tap targets, drop the table
   in favor of stacked cards so nobody has to scroll a table sideways with a finger. */
@media (max-width: 860px) {
  .nav { width: 100%; order: 3; justify-content: flex-start; }
  .plate { order: 1; }
  .switch { order: 2; margin-left: auto; }

  .filter-bar { flex-direction: column; align-items: stretch; }
  .field { width: 100%; max-width: none; }
  ...
  /* Table becomes a stack of record cards */
